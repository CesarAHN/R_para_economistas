...
Pero esa no es mi carpeta que contiene mis archivos que deseo importar, por lo cual me surge la necesidad de cambiar el directorio de trabajo[^1]. La ruta del directorio de trabajo se cambia con la función `setwd()`, al interior de los paréntesis se coloca la ruta, en mi caso la ruta está en `C:\RRRR\R_para_economistas\Archivos`, por lo que tendré que colocar aquello dentro de los paréntesis, pero cambiando los `\` por `/`, esto debido a que R usa el sistema usado por **GNU/linux**. 
...
library(data.table)
df<-fread("Archivos/votos.txt", header=T, verbose=F)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Leyendo archivo csv.
df<-read.csv("Archivos/votos.csv", header = T, sep = ";")
...
## Importar archivos desde excel (xlsx).

Hay una gran variedad de paquetes que nos permiten importar archivos en formato *.xlsx*. En esta guía se usarán dos: `openxlsx` y `readxl`. Cada paquete tiene una ventaja respecto al otro y viceversa. Por un lado el paquete `openxlsx` solo sirve para archivos de formato ".xlsx", mientras que el paquete también puede leer archivos ".xls". Por otro lado, el paquete`readxl` tiene dificultades para leer caracteres extraños del archivo excel. Hay más diferencias que usted se podrá ir dando cuenta con el uso que le da y dependerá de lo que desea cargar. Particularmente, tengo una parcialidad hacia el paquete `openxlsx`, pero eso no significa que el paquete `readxl` sea innecesario. 
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Leyendo archivo xlsx.
df<-read.xlsx("Archivos/votos.xlsx")
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Leyendo archivo .xlsx
df<-read_xlsx("Archivos/votos.xlsx")
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Leyendo archivo .sav
df<-read.spss("Archivos/votos.sav", to.data.frame = T)
...
## Importar archivos desde STATA.

Para importar archivos con extensión *".dta"* nosotros podemos usar dos paquetes: `foreign` o `haven`. Nosotros recomendamos usar el paquete `haven` ya que la función que lee archivos ".dta" lee archivos de versiones superiores[^3]. 

[^3]: El paquete `foreign` solo lee archivos stata de la versión 5 a la 12, mientras que `haven` lee archivos hasta la versión 16 del stata.
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Leyendo archivo .dta
df<-read_dta("Archivos/votos.dta")
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Leyendo archivo .dbf
df<-read.dbf("Archivos/votos.dbf")
