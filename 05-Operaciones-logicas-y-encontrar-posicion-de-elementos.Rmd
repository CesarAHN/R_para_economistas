# OPERADORES LÓGICOS, ÍNDICE DE ELEMENTOS Y CAMBIO DE VALORES. 
...
## ENCONTRAR EL ÍNDICE DE ELEMENTOS Y CAMBIAR VALORES.
...
`x` = Es un vector lógico o un array.
`arr.ind` = Argumento lógico que se usa cuando `x` es un array. Si es TRUE, entonces, devuelve los índices en una matriz cuando x es una matriz.
`ùseNames` = Argumento lógico que se usa cuando `x` es una array. Si es TRUE, entonces, los nombres del array se deben de mostrar.
...
Lo que nos ha arrojado R son las posiciones de los elementos que son mayores a 50 del vector `x`. Es así que el elemento de posición `r which(x>50)[1]` es mayor a 50. Si nos fijamos en el vector x, el elemento de posición 4 es el elemento de valor `r x[4]` que es mayor a 50.
